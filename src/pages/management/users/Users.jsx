...
import { useDeleteAdminRepoMutation, useGetAdminRepoQuery } from '@/app/features/admin-apis/admin-repo-api-slice';
import HeadingNav from '@/components/heading-nav';
import AddIcon from '@mui/icons-material/Add';
import DeleteIcon from '@mui/icons-material/Delete';
import EditIcon from '@mui/icons-material/Edit';
import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Paper, Skeleton, TableContainer, Typography } from '@mui/material';
...
const Users = () => {
  ...
  const { data, isError, isLoading, error , isSuccess } = useGetAdminRepoQuery("user", { skip: !token });
  ...
  if (isObjectError || isError) {
    const message = objectError?.data?.message || objectError?.data?.error || error?.data?.message || error?.data?.error;
    toast.error(message);
    return null;
  }
  ...
  const keysOfObject = Object.keys(objectData.properties).filter(key => objectData.properties[key].display_ui);
  ...
  columns = [{ field: 'id', headerName: 'ID', width: 90, editable: false }, ...columns];
...
<Box>
  ...
  <Typography variant="subtitle1" sx={{ mb: 2 }}> 
    {objectData.description}
      ...
      <TableContainer sx={{ height: 500, width: '100%', p: 1 }} component={Paper}>
        <DataGrid
        sx={{
          cursor: 'pointer',
        }}
